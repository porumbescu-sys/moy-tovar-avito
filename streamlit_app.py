...
def render_crm_header_bar(
    ...
    st.markdown('<div class="result-wrap">', unsafe_allow_html=True)
    render_block_header(
        "CRM — быстрый центр управления [PHOTO/AVITO TOOLS TEST 777]",
        "Короткая шапка с главными рабочими зонами: задачи, выгодные закупки, проблемы по карточкам. Если ты видишь TEST 777 — сервис запущен из нового файла.",
        icon="🧭",
        help_text="Эта шапка ничего не меняет в comparison. Она только помогает быстро открыть нужный рабочий блок без долгой прокрутки. Маркер TEST 777 нужен только для проверки загрузки нового файла.",
    )
    st.caption("🔎 Маркер проверки: если эта строка видна, сервис реально подхватил новый файл с CRM-инструментами.")
