...
def watch_is_buy_candidate(rec: Any, threshold_pct: float = 35.0) -> bool:
    if rec is None:
        return False
    ...
    if not isinstance(rec, dict):
        return False
    our_price = safe_float(rec.get("our_price_now"), 0.0)
    supplier_price = safe_float(rec.get("best_supplier_price_now"), 0.0)
    gap_pct = safe_float(rec.get("best_supplier_gap_pct"), 0.0) * 100.0
    supplier_name = normalize_text(rec.get("best_supplier", ""))
    return (
        our_price > 0
        and supplier_price > 0
        and bool(supplier_name)
        and gap_pct >= float(threshold_pct)
    )
...
def hot_watchlist_summary_text() -> str:
    ...
    buy_count = int(sum(1 for _, rec in hot_df.iterrows() if watch_is_buy_candidate(rec, threshold_pct=35.0)))
    ab_count = int(hot_df.get("abc_class", pd.Series(dtype=object)).fillna("").map(normalize_text).isin(["A", "B"]).sum())
    return f"Ходовых: {len(hot_df)} • сильный спрос: {ab_count} • можно брать сейчас: {buy_count}"
...
def build_hot_buy_watchlist_table() -> pd.DataFrame:
    ...
    if not isinstance(hot_df, pd.DataFrame) or hot_df.empty:
        return pd.DataFrame()
    work = hot_df.copy()
    buy_mask = pd.Series([watch_is_buy_candidate(rec, threshold_pct=35.0) for _, rec in work.iterrows()], index=work.index)
...
def render_hot_buy_watchlist_lazy_panel() -> None:
    ...
    render_block_header(
        ...
        "Ленивая таблица только по ходовым позициям, где лучший поставщик сейчас минимум на 35% дешевле нашей цены. Считается и показывается только когда включён чекбокс в sidebar.",
        icon="🔥",
    ...
    if buy_df.empty:
        ...
        c1.metric("Позиций можно брать сейчас", len(buy_df))
        c2.download_button(
...
with st.sidebar:
    ...
    st.markdown(f'<div class="sidebar-mini">Файл на сервере: {html.escape(str(get_persisted_watchlist_file_path()))}</div>', unsafe_allow_html=True)    st.markdown(f'<div class="sidebar-mini">{html.escape(hot_watchlist_summary_text())}</div>', unsafe_allow_html=True)
    hot_df_state = st.session_state.get("hot_items_df")
    hot_buy_total = 0
    if isinstance(hot_df_state, pd.DataFrame) and not hot_df_state.empty:
        hot_buy_total = int(sum(1 for _, rec in hot_df_state.iterrows() if watch_is_buy_candidate(rec, threshold_pct=35.0)))
    if hot_buy_total > 0:
        st.checkbox(
            f"Показать таблицу «можно брать сейчас» ({hot_buy_total})",
            key="show_hot_buy_watchlist_table",
            help="Показывает только ходовые позиции, где лучший поставщик сейчас минимум на 35% дешевле нашей цены.",
        )
        st.markdown(
            '<div class="sidebar-mini">Таблица откроется только по позициям, где поставщик сейчас минимум на 35% дешевле нашей цены.</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div class="sidebar-mini">Сейчас позиций для выгодной закупки не найдено.</div>',
            unsafe_allow_html=True,
        )
    st.markdown('</div>', unsafe_allow_html=True)
